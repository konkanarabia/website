...
export default function EditEventAdmin({ params }: { params: Promise<{ id: string }> }) {
      ...
      try {
        ...
        if (data.success && data.event) {
          ...
          setFormData({
            ...
            enquiryLink: s.enquiryLink || '/enquiry/event',
  ...
  const handleSave = async () => {
    if (!formData.name) return toast.error('Event Name is required');
    ...
    try {
      ...
      const result = await updateEvent(id, fd);
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 md:my-10 bg-white rounded-xl shadow-lg border border-gray-100 font-sans">
      <div className="flex items-center justify-between mb-8">
        <Link href="/admin?tab=events" className="text-gray-500 hover:text-gray-800 flex items-center gap-2 font-medium">
