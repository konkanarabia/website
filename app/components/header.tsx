...
export default function Header() {
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4 flex justify-between items-center">
          <div className="text-sm flex items-center">
            ...
            <span className="flex items-center">
              <Mail className="h-4 w-4 mr-1" /> info@konkanarabiahospitalitygroup.com
            </span>
          </div>
          <div className="flex space-x-2">
            <Link href="https://www.facebook.com/konkanarabiahospitalitygroup" className="hover:text-gray-200 transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-facebook" viewBox="0 0 16 16">
                <path d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951z"/>
              </svg>
            </Link>
            <Link href="https://www.instagram.com/konkanarabiaholidays" className="hover:text-gray-200 transition-colors">
      ...
      <header className={`bg-white ${isScrolled ? 'shadow-md sticky top-0 z-50 py-2' : 'py-4'} transition-all duration-300`}>
        <div className="container mx-auto px-4 flex justify-between items-center">
          <Link href="/" className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xl font-bold text-primary hidden sm:block leading-tight">
                KonkanArabia Hospitality & Holiday Group
              </span> 
              <span className="text-sm text-gray-600 hidden sm:block leading-tight">Hospitality is Our Passion & Culture</span>
            </div>
          </Link>
          ...
          <div className="flex items-center justify-end flex-1">
            <nav className="hidden md:block ml-auto">
              <ul className="flex space-x-1 lg:space-x-2 items-center">
                    ...
                    {item.dropdown ? (
                      <div ref={dropdownRef}>
                        <button
                          ...
                          className={`flex items-center text-sm lg:text-base px-3 py-2 rounded-md hover:bg-primary/10 transition-colors ${
                            pathname.startsWith(item.href) ? "font-semibold text-primary" : "text-gray-700"
                          }`}
                        >
                          {item.name}
                      ...
                      <Link
                        ...
                        className={`text-sm lg:text-base px-3 py-2 rounded-md hover:bg-primary/10 transition-colors block ${
                          pathname === item.href ? "font-semibold text-primary" : "text-gray-700"
                        }`}
                      >
                        {item.name}
                      </Link>
                ...
                <li>
                  <Link href="/enquiry">
                    <Button size="sm" className="bg-primary hover:bg-primary/90 text-white ml-2">
                      Enquire Now
                    </Button>
                  </Link>
                </li>
            ...
            <nav className="bg-white px-4 pt-2 pb-4 shadow-inner border-t border-gray-100">
              <ul className="space-y-2">
                {navItems.map((item) => (
                  <li key={item.name}>
                    {item.dropdown ? (
                      <div ref={mobileDropdownRef} className="border-b border-gray-100 pb-2">
                        <button
