...
export async function getPaginatedRestaurants(page: number = 1, limit: number = 9) {
  await dbConnect();
  try {
    const skip = (page - 1) * limit;
    const restaurants = await Restaurant.find().sort({ id: 1 }).skip(skip).limit(limit).lean();
  ...
  try {
    ...
    if (file && file.size > 0) {
      ...
      imageUrl = await new Promise((resolve, reject) => {
        cloudinary.uploader.upload_stream({ folder: 'restaurants' }, (err: any, result: any) => {
          if (err) reject(err);
    ...
    const lastDest = await Restaurant.findOne().sort({ id: -1 });
    ...
    if (imageUrl) {
      ...
      try {
          const result = await cloudinary.uploader.upload(aiImageUrl, { folder: 'restaurants' });
...
export async function updateRestaurant(id: string | number, formData: FormData) {
  await dbConnect();
